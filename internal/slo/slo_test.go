...
func TestErrorBudgetRemaining(t *testing.T) {
	tests := []struct {
		...
		{"half budget gone", 99.9, 99.95, 50.0},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if math.Abs(got-tt.wantApprox) > 2.0 {
...
func TestIsCritical(t *testing.T) {
	if !IsCritical(14.4) {
		t.Error("14.4x should be critical")
	}
	if !IsCritical(20.0) {
		t.Error("20.0x should be critical")
	}
	if IsCritical(6.0) {
		t.Error("6.0x should not be critical")
	}
}
